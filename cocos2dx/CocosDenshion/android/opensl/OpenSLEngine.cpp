...
bool initAudioPlayer(AudioPlayer* player, const char* filename) 
{
	...
	off_t start, length;
//    LOGD("initAudioPlayer PlayerCnt: %d", g_iPlayerCnt);
    if (g_iPlayerCnt >= 30)
    {
        return false;
    }
	...
	if (FILE_NOT_FOUND == fd)
	{
		...
		if(fp){
			...
			bool ret = createAudioPlayerBySource(player);
            if (ret)
            {
                ++g_iPlayerCnt;
            }
...
void destroyAudioPlayer(AudioPlayer * player)
{
	if (player && player->fdPlayerObject != NULL)
	{
        ...
        delete player;
        // LOGD("destroyAudioPlayer PlayerCnt: %d", g_iPlayerCnt);
	}
}
...
class CSyncList: public std::vector<CallbackContext*>
{
public:
    typedef std::vector<CallbackContext*>::iterator iterator;
    typedef std::vector<CallbackContext*>::const_iterator const_iterator;
    
    CSyncList();
    ~CSyncList();
    
    void push_back(CallbackContext*);
    void pop_back();
    pthread_mutex_t * GetMutex();
    
private:
    typedef std::vector<CallbackContext*> Super;
    mutable pthread_mutex_t m_Mutex;
};

CSyncList g_garbageList;
...
void CSyncList::push_back(CallbackContext* pContext)
{
    pthread_mutex_lock(&m_Mutex);
    Super::push_back(pContext);
    pthread_mutex_unlock(&m_Mutex);
}
void CSyncList::pop_back()
{
    pthread_mutex_lock(&m_Mutex);
    Super::pop_back();
    pthread_mutex_unlock(&m_Mutex);
...
pthread_mutex_t * CSyncList::GetMutex()
{
    return &m_Mutex;
}
...
void PlayOverEvent(SLPlayItf caller, void* pContext, SLuint32 playEvent)
{
	...
	{
        LOGD("PlayOverEvent SL_PLAYEVENT_HEADATEND: %d ", context->player);
        ...
        g_garbageList.push_back(context);
...
//		delete context;
    ...
    {
      //  LOGD("PlayOverEvent event:%d: %d ", playEvent, context->player);
	}
}
...
bool OpenSLEngine::recreatePlayer(const char* filename)
{
	...
	if (!initAudioPlayer(newPlayer, filename))
	{
        ...
        if (vec->size() > 1)
        {
            LOGD("recreatePlayer() retry");
            AudioPlayer* pOldPlayer = vec->at(vec->size() - 1);
            destroyAudioPlayer(pOldPlayer);
            vec->pop_back();
           if (!initAudioPlayer(newPlayer, filename))
           {
               LOGD("recreatePlayer() retry failed");
               delete newPlayer;
               newPlayer = NULL;
		       return false;
	        }
...
{
    pthread_mutex_lock(g_garbageList.GetMutex());
    vector<CallbackContext*>::iterator i;
    for (i = g_garbageList.begin(); i != g_garbageList.end(); ++i)
    {
        CallbackContext* pContext = *i;
        destroyAudioPlayer(pContext->player);
        delete pContext;
//        LOGD("preloadEffect context deleted. size: %d", g_garbageList.size());
    }    
    g_garbageList.clear();    
    pthread_mutex_unlock(g_garbageList.GetMutex());
