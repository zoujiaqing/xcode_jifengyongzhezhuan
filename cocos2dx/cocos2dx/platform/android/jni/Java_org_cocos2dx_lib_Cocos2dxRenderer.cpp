...
extern "C" {
	...
	JNIEXPORT void JNICALL Java_org_cocos2dx_lib_Cocos2dxRenderer_nativeonWindowFocusChanged(JNIEnv* env, jobject thiz, jboolean HasFocus) {
	     unsigned char b = HasFocus;    
	     CCLog("Java_org_cocos2dx_lib_GLThreadManager_nativeonWindowFocusChanged %lu",b );
		 
        if (CCDirector::sharedDirector()->getOpenGLView()) {
            CCApplication::sharedApplication()->applicationWindowFocusChanged(b);
        }
    }
