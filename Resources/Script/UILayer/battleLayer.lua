...
local function initHeroInfo()
	...
	heroInfo:setCurrenSectRagePoint(UserData:getSecRagePoint());
	heroInfo:setTotalSecRagePoint(UserData:getSecTotalRagePoint());
	...
	--Note: 第二个怒气条
	local secHeroRageBar = battleLayout:FindChildObjectByName("secRageBar").__UIPicture__:getCurrentNode();
	if secHeroRageBar ~= nil then
		secHeroRageBar:setVisible(false);
		local ptx,pty = secHeroRageBar:getPosition();
		secHeroRageBar:setAnchorPoint(ccp(0,0.5));
		local scale =  TXGUI.UIManager:sharedManager():getScaleFactor();
		secHeroRageBar:setPosition(ccpSub(ccp(ptx,pty),ccp(scale * secHeroRageBar:getContentSize().width * 0.5,0)));
		heroInfo:setSecRageBar(secHeroRageBar);
		local ragePointScale = scale * UserData:getSecRagePoint() / UserData:getSecTotalRagePoint();
		secHeroRageBar:setScaleX(ragePointScale)
	end

	local secHeroRageBackBar = battleLayout:FindChildObjectByName("secRageBackBar").__UIPicture__:getCurrentNode();
	if secHeroRageBackBar ~= nil then
		secHeroRageBackBar:setVisible(false);
		local ptx,pty = secHeroRageBackBar:getPosition();
		secHeroRageBackBar:setAnchorPoint(ccp(0,0.5));
		local scale =  TXGUI.UIManager:sharedManager():getScaleFactor();
		secHeroRageBackBar:setPosition(ccpSub(ccp(ptx,pty),ccp(scale * secHeroRageBackBar:getContentSize().width * 0.5,0)));
		heroInfo:setSecRageBackBar(secHeroRageBackBar);
	end

	local secIconRage = battleLayout:FindChildObjectByName("secIconRage").__UIPicture__:getCurrentNode();
	if secIconRage ~= nil then
		secIconRage:setVisible(false);		
		local ptx,pty = secIconRage:getPosition();
		secIconRage:setAnchorPoint(ccp(0,0.5));
		local scale =  TXGUI.UIManager:sharedManager():getScaleFactor();
		secIconRage:setPosition(ccpSub(ccp(ptx,pty),ccp(scale * secIconRage:getContentSize().width * 0.5,0)));
		heroInfo:setSecRageBall(secIconRage);
		
	end

	local secIconRageFull = battleLayout:FindChildObjectByName("secIconRageFull").__UIPicture__:getCurrentNode();
	if secIconRageFull ~= nil then
		secIconRageFull:setVisible(false);		
		local ptx,pty = secIconRageFull:getPosition();
		secIconRageFull:setAnchorPoint(ccp(0,0.5));
		local scale =  TXGUI.UIManager:sharedManager():getScaleFactor();
		secIconRageFull:setPosition(ccpSub(ccp(ptx,pty),ccp(scale * secIconRageFull:getContentSize().width * 0.5,0)));
		heroInfo:setSecRageBallFull(secIconRageFull);		
	end
...
local function updateHeroRagePoint()
	...
	if (layout ~= nil) and (heroInfo ~= nil) then
		...
		--print("^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^ updateHeroRagePoint " .. UserData:getSecRagePoint())
		heroInfo:playSecRageAni(UserData:getSecRagePoint());
	end
...
backBtn:registerScriptTapHandler(onBackTown);
 ...
 function unRegisterBattleHandlers()
...
local function updatePPVEPlayerHP()
	...
	local isGuildInstance = LevelManager:sShareInstance():isCurrentGuildInstanceLevel();
	if isPPVE or isCityDefend or isGuildInstance then
		...
		for index,value in pairs(players) do
			if value ~= nil then
				...
				if info == nil then
					...
					player:setCurrenSectRagePoint(value.secCurrentRagePoint);
					player:setTotalSecRagePoint(value.secTotalRagePoint);
					...
					if otherPlayer:getCurrentSecRagePoint() ~= value.secCurrentRagePoint then		
						otherPlayer:playSecRageAni(value.secCurrentRagePoint);
					end
				end
