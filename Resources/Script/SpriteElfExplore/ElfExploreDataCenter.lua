...
function ElfExploreDataCenter:ExploreRewardCommit()
	...
	for index,value in pairs(self.ExploreReward.rewards) do
		...
		if TellIsElfId(item_id) then
			local elfFlagName = "";
			if Localization['ELF_NAME'] ~= nil then
				elfFlagName = Localization['ELF_NAME'].SimpleChinese;
			end			
			local elfName = GetOneElfName(item_id);
			rewardInfo = rewardInfo .. "  " .. elfFlagName .. " " .. elfName
			--Note:更新精灵，新得到的精灵
			--print("SpriteElfDataCenter:Get():SendSetElfStateMessage(item_id,2)")
			--SpriteElfDataCenter:Get():SendSetElfStateMessage(item_id,2)
			ElfExploerLayer:ShowLeftScrollList()
		else
			local name = Items[item_id].Name
			local content =  Localization[name].SimpleChinese
			rewardInfo = rewardInfo .. "  " .. content .. "*" .. item_num
		end
	end
...
function ElfExploreDataCenter:PushExploreNums(Nums)	
	self.ExploreTimes = Nums
	ElfExploerLayer:ShowExploreResfashNumsInfoPanel()
end
...
function ElfExploreDataCenter:GetMapElfIdAndEventId(id)
	...
	if nil ~= _start and 1 ~= _start then
		...
		print("elf_id " .. elf_id)
		if LuaSpriteElfDataCenter:IsOneElfExistInElfDataTable(elf_id) then
			return nil ,nil
		end
...
function PushExchangeElfRet(elfId)
	...
	--Note:兑换精灵
	LuaSpriteElfDataCenter:SetFairyState(elfId,2)
