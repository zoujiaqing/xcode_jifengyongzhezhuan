...
_pbcP_push_enum(struct pbc_env * p, const char *name, struct map_kv *table, int sz) {
	...
	if (check)
		return NULL;
	v = (struct _enum *)_pbcM_malloc(sizeof(*v));
	v->key = name;
...
_pbcP_init_message(struct pbc_env * p, const char *name) {
	...
	iter.table = (struct map_kv *)_pbcM_malloc(iter.count * sizeof(struct map_kv));
	iter.count = 0;
...
int 
pbc_type(struct pbc_env * p, const char * name , const char * key , const char ** type) {
	struct _message *m = _pbcP_get_message(p, name);
	if (m==NULL) {
		return 0;
	}
	if (key == NULL) {
