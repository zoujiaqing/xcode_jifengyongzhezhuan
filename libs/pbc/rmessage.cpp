	...
	for (i=0;i<ctx->number;i++) {
		...
		if (f) {
			if (f->label == LABEL_REPEATED || f->label == LABEL_PACKED) {
				...
				struct value * v =(struct value *) read_value(h, f, &(ctx->a[i]), (uint8_t *)buffer);
				if (v) {
...
struct pbc_rmessage * 
pbc_rmessage_new(struct pbc_env * env, const char * name ,  struct pbc_slice * slice) {
...
pbc_rmessage_message(struct pbc_rmessage * rm, const char *key, int index) {
	...
	if (v == NULL) {
		...
		if (m->def == NULL) {
			...
			m->def = (pbc_rmessage *)_pbcM_malloc(sizeof(struct pbc_rmessage));
			m->def->msg = m;
			m->def->index = NULL;
		}
		return m->def;
