...
int pbc_type(struct pbc_env *, const char * name , const char * key , const char ** type);
...
typedef void (*pbc_decoder)(void *ud, int type, const char * name, union pbc_value *v, int id, const char *key);
int pbc_decode(struct pbc_env * env, const char * type , struct pbc_slice * slice, pbc_decoder f, void *ud);

// message api

struct pbc_rmessage * pbc_rmessage_new(struct pbc_env * env, const char * type , struct pbc_slice * slice);
...
struct pbc_wmessage * pbc_wmessage_new(struct pbc_env * env, const char *type);
