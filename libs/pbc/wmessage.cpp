...
struct pbc_wmessage * 
pbc_wmessage_new(struct pbc_env * env, const char *name) {
...
#undef _expand
static void _expand(struct pbc_wmessage *m, int sz) {
...
pbc_wmessage_string(struct pbc_wmessage *m, const char *key, const char * v, int len) {
	...
	if (f->label == LABEL_PACKED) {
		if (f->type == PTYPE_ENUM) {
			char *temp = (char *)_pbcM_malloc(len+1);
			if (!varlen || v[len] != '\0') {
				memcpy(temp,v,len);
				temp[len]='\0';
				v = temp;
...
_pack_packed(void *p, void *ud) {
	...
	struct pbc_wmessage * m = (pbc_wmessage *)ud;
...
pbc_wmessage_buffer(struct pbc_wmessage *m, struct pbc_slice *slice) {
	...
	for (i=0;i<n;i++) {
		...
		struct pbc_slice s;
		pbc_wmessage_buffer((pbc_wmessage *)var->p[0] , &s);
		if (s.buffer) {
			struct _field * f = (_field *)var->p[1];
