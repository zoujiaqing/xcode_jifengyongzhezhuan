...
#define luaL_prepbuffsize( b , cap ) _pbcM_malloc(cap)
...
_rmessage_new(lua_State *L) {
	...
	struct pbc_rmessage * m = pbc_rmessage_new(env, name, &slice);
	if (m==NULL)
		return 0;
...
_env_type(lua_State *L) {
	...
	const char * name = (const char *)luaL_checkstring(L,2);
	if (lua_isnil(L,3)) {
		int ret = pbc_type(env, name, NULL, NULL);
		lua_pushboolean(L,ret);
		return 1;
	}
	const char * key = luaL_checkstring(L,3);
	const char * type = NULL;
	int ret = pbc_type(env, name, key, &type);
...
_wmessage_new(lua_State *L) {
	struct pbc_env * env = (struct pbc_env * )checkuserdata(L,1);
	const char * name = (const char * )luaL_checkstring(L,2);
	struct pbc_wmessage * ret = pbc_wmessage_new(env, name);
...
_pattern_unpack(lua_State *L) {
	...
	char  *temp = (char *)_pbcM_malloc(size);
...
_pattern_pack(lua_State *L) {
	...
	char *data = (char *)_pbcM_malloc(size);
...
static void
push_value(lua_State *L, int type, const char * name, union pbc_value *v) {
	switch(type) {
	case PBC_INT:
	...
	default:
		luaL_error(L, "Unknown type %s",(const char * )name);
		break;
	}
}
 ...
 */
static void
decode_cb(void *ud, int type,const char * name, union pbc_value *v, int id, const char *key) {
	...
	if (type & PBC_REPEATED) {
		push_value(L, type & ~PBC_REPEATED,(const char * )name, v);
		...
		push_value(L, type,(const char * )name, v);
...
_gc(lua_State *L) {
	...
	obj->size_msg = 0;
	obj->cap_msg = 4;
	obj->pat = (struct pbc_pattern **)_pbcM_malloc(obj->cap_pat * sizeof(struct pbc_pattern *));
	obj->msg = (pbc_rmessage **)_pbcM_malloc(obj->cap_msg * sizeof(struct pbc_rmessage *));
...
_add_pattern(lua_State *L) {
	...
	if (obj->size_pat >= obj->cap_pat) {
		obj->cap_pat *= 2;
		obj->pat = (pbc_pattern **)realloc(obj->pat, obj->cap_pat * sizeof(struct pbc_pattern *));
	}
...
_add_rmessage(lua_State *L) {
	...
	if (obj->size_msg >= obj->cap_msg) {
		obj->cap_msg *= 2;
		obj->msg = (pbc_rmessage **)realloc(obj->msg, obj->cap_msg * sizeof(struct pbc_rmessage *));
	}
	struct pbc_rmessage * msg = (pbc_rmessage *)lua_touserdata(L,2);
