...
call_unknown(pbc_decoder f, void * ud, int id, struct atom *a, uint8_t * start) {
	...
	case WT_VARINT:
		v.i.low = a->v.i.low;
		v.i.hi = a->v.i.hi;
		f(ud, PBC_INT, nametype[PBC_INT], &v, id , NULL);
		break;
	case WT_BIT64:
		v.i.low = a->v.i.low;
		v.i.hi = a->v.i.hi;
		f(ud, PBC_FIXED64, nametype[PBC_FIXED64], &v, id , NULL);
		break;
	case WT_LEND:
		v.s.buffer = (char*)start + a->v.s.start;
		v.s.len = a->v.s.end - a->v.s.start;
		f(ud, PBC_BYTES, nametype[PBC_BYTES], &v, id , NULL);
		break;
	case WT_BIT32:
		v.i.low = a->v.i.low;
		v.i.hi = 0;
		f(ud, PBC_FIXED32, nametype[PBC_FIXED32], &v, id , NULL);
		break;
	default:
		return 1;
	}
...
call_type(pbc_decoder pd, void * ud, struct _field *f, struct atom *a, uint8_t * start) {
	...
	int type = _pbcP_type(f, &name);
	assert(type != 0);
	if (name == NULL) {
		name =(const char *) nametype[type & ~PBC_REPEATED];
	}
	switch (f->type) {
	case PTYPE_DOUBLE:
		CHECK_BIT64(a, -1);
...
int
pbc_decode(struct pbc_env * env, const char * name , struct pbc_slice * slice, pbc_decoder pd, void *ud) {
	struct _message * msg = _pbcP_get_message(env, name);
	if (msg == NULL) {
